...
import { fetchAllEmailDomains } from "@/services/emaildomains";
...
import AuthorityDrawer from "./form";
import {
  createCompanies,
  fetchCompanies,
  updateCompanies,
...
export default function EmailDomains() {
  ...
  const {
    ...
    queryKey: ["companies", filterOptions],
    queryFn: () =>
      fetchCompanies({
        isActive: true,
        ...filterOptions,
  ...
  const Mutation = useMutation({
    mutationFn: (values) =>
      mode == "create" ? createCompanies(values) : updateCompanies(values),
    onSuccess: () => {
  ...
  return (
    <>
      <ComplexTable
        ...
        paginationConfig={{
          current: companies?.pagination.current,
          pageSize: companies?.pagination.pageSize,
      ...
      <MapDrawer
        ...
        drawerContent={<AuthorityDrawer mapDrawerOpen={mapDrawerOpen} onSubmit={onSubmit} />}
      ></MapDrawer>
    </>
  );
}
