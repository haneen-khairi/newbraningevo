...
import { createHeadquarters, fetchHeadquarters, updateHeadquarters } from "../../../services/headquarter";
import HeadQuartersDrawer from "./form";
...
export default function HeadQuarters() {
  ...
  const columns = [
    {
      title: t("building"),
      dataIndex: ["building", "nameAr"],
      key: "day",
      render: (v, record) =>
        i18n.language == "ar" ? v : record?.building?.nameEn,
    },
    {
      title: t("floor"),
      dataIndex: ["floor", "nameAr"],
      key: "day",
      render: (v, record) =>
        i18n.language == "ar" ? v : record?.floor?.nameEn,
    },
    {
      title: t("company"),
      dataIndex: ["company", "nameAr"],
      key: "day",
      render: (v, record) =>
        i18n.language == "ar" ? v : record?.company?.nameEn,
    },
    {
      title: t("authority"),
      dataIndex: ["side", "nameAr"],
      key: "day",
      render: (v, record) => (i18n.language == "ar" ? v : record?.side?.nameEn),
    },
    {
      title: t("room"),
      dataIndex: ["room", "nameAr"],
      key: "day",
      render: (v, record) => (i18n.language == "ar" ? v : record?.room?.nameEn),
    },
    {
      title: t("lastUpdate"),
      dataIndex: "updatedAt",
  ...
  const Mutation = useMutation({
    mutationFn: (values) =>
      mode == "create" ? createHeadquarters(values) : updateHeadquarters(values),
    onSuccess: () => {
  ...
  return (
    <>
      <ComplexTable
        tableTitle={t("headquarters")}
        addText={t("addHeadquarters")}
        columns={columns}
      ...
      <MapDrawer
        ...
        drawerContent={<HeadQuartersDrawer mapDrawerOpen={mapDrawerOpen} onSubmit={onSubmit} />}
      ></MapDrawer>
    </>
  );
}
