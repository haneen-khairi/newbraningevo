...
export default function Floors() {
  ...
  const Mutation = useMutation({
    mutationFn: (values) =>
      formState == "create" ? createFloor(values) : updateFloor(values),
    onSuccess: (data) => {
  ...
  const onSubmit = (values, mode) => {
    values.buildingId = values["originBuildingId-coordinates"].id;

    delete values["originBuildingId-coordinates"];
    delete values["originBuildingId-label"];

    !mode ? "" : setFormState(mode);
  ...
  return (
    <>
      <ComplexTable
        ...
        paginationConfig={{
          current: floors?.pagination.current,
          pageSize: floors?.pagination.pageSize,
          total: floors?.pagination.total,
        }}
      ...
      <MapDrawer
        ...
        drawerContent={
          <FloorDrawer isOpen={mapDrawerOpen} onSubmit={onSubmit} />
        }
      />
    </>
  );
