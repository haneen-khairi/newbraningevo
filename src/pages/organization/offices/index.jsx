...
import { createRoom, fetchAllRooms, updateRoom } from "../../../services/rooms";
import OfficeDrawer from "./form";
...
export default function Office() {
  ...
  const Mutation = useMutation({
    mutationFn: (values) =>
      formState == "create" ? createRoom(values) : updateRoom(values),
    onSuccess: (data) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "lastUpdate",
      sorter: true,
      render:(v)=>dayjs(v).locale(i18n.language).format("HH:MM    DD MMMM YY")
    },
    {
      title: t("action"),
      dataIndex: "actions",
...
<MapDrawer
        ...
        footer={
          <Button type="primary" htmlType="submit" form="office-form" className="w-full rounded-xl">
            {t("save")}
          </Button>
        }
        drawerContent={
          <OfficeDrawer isOpen={mapDrawerOpen} onSubmit={onSubmit}/>
        }
      >
      </MapDrawer>
    </>
